...
export const groupService = {
  list(): Group[] {
    return getDb().prepare('SELECT * FROM groups ORDER BY created_at DESC').all() as Group[]
  },

  getById(id: string): Group | null {
    return (getDb().prepare('SELECT * FROM groups WHERE id = ?').get(id) as Group | undefined) ?? null
  },
